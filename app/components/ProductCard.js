...
function ProductCard(props) {
    ...
    return (
        <>
            <div className="w-64 lg:w-xs">
                <Link href={slug} className='flex justify-end relative top-16 left-10'>
                    <img src={src} alt={alt} className="w-36 lg:w-48" />
                </Link>
                <div className='bg-white border-0 rounded-lg shadow py-5 pl-6'>
                    <span className="text-pizza-red font-bold text-sm lg:text-lg pl-3 relative bottom-3 font-[inter]">From {price}</span>
                    <h4 className="mt-3 font-bold text-lg lg:text-2xl">{name}</h4>
                    <p className="text-gray-500 font-light text-sm lg:text-lg">{subtitle}</p>
